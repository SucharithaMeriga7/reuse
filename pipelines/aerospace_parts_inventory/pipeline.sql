...
-- Purpose     : Incremental/full SCD Type 1 load from
--               AEROSPACE_PARTS_SOURCE to AEROSPACE_PARTS_TARGET
--               with deduplication, soft-delete, RISK_SCORE
--               derivation, and mandatory reconciliation logging
-- Author      : SUCHARITHAS
-- Generated   : 2026-09-24
-- [DELTA REFERENCE] Adapted from: AEROSPACE_PARTS_INVENTORY_PIPELINE
--                   (Object Registry, 68% match, ADAPT route)
-- ============================================================
...
DECLARE
    ...
    v_pipeline_run_watermark   TIMESTAMP_NTZ;  -- [DELTA ADD] PIPELINE_RUN_WATERMARK: captures watermark used for this run
...
BEGIN
    ...
    IF (P_MODE = 'FULL') THEN
        ...
        v_pipeline_run_watermark := '1900-01-01'::TIMESTAMP_NTZ;  -- [DELTA ADD] PIPELINE_RUN_WATERMARK: set for full-load mode
        v_load_mode              := 'FULL';
    ELSE
        ...
        v_pipeline_run_watermark := :v_last_ts;  -- [DELTA ADD] PIPELINE_RUN_WATERMARK: capture watermark for this incremental run
    ...
    SELECT
        PART_NUMBER,
        INITCAP(MANUFACTURER)                                    AS MANUFACTURER,           -- [DELTA MODIFY] MANUFACTURER: UPPER to INITCAP per BRD CamelCase rule
        IFF(WEIGHT_KG <= 0, -1, WEIGHT_KG)                      AS WEIGHT_KG,              -- [DELTA MODIFY] WEIGHT_KG: CASE WHEN to IFF sentinel replace per BRD
        CERTIFICATION_STATUS,
        LEAD_TIME_DAYS,
        UPDATED_AT,
        SUPPLIER_RATING,                                                                     -- [DELTA ADD] SUPPLIER_RATING: direct load from source, no transformation per BR-008
        :v_pipeline_run_watermark                                AS PIPELINE_RUN_WATERMARK, -- [DELTA ADD] PIPELINE_RUN_WATERMARK: watermark TS used for this run extract
        CASE
    ...
    WHEN MATCHED THEN UPDATE SET
        ...
        tgt.SUPPLIER_RATING        = src.SUPPLIER_RATING,              -- [DELTA ADD] SUPPLIER_RATING: added to MERGE UPDATE per BR-008
        tgt.PIPELINE_RUN_WATERMARK = src.PIPELINE_RUN_WATERMARK,      -- [DELTA ADD] PIPELINE_RUN_WATERMARK: added to MERGE UPDATE
        tgt.STATUS                 = 'Active',                          -- [DELTA RENAME] STATUS: was RECORD_STATUS, renamed to STATUS per BRD/STTM
        tgt.LOAD_TIMESTAMP         = CURRENT_TIMESTAMP()               -- [DELTA RENAME] LOAD_TIMESTAMP: was ETL_LOAD_TIMESTAMP, renamed to LOAD_TIMESTAMP per BRD/STTM
    WHEN NOT MATCHED THEN INSERT (
        PART_NUMBER, MANUFACTURER, WEIGHT_KG, CERTIFICATION_STATUS,
        LEAD_TIME_DAYS, UPDATED_AT, RISK_SCORE,
        SUPPLIER_RATING, PIPELINE_RUN_WATERMARK,                        -- [DELTA ADD] SUPPLIER_RATING, PIPELINE_RUN_WATERMARK: added to INSERT list
        STATUS, LOAD_TIMESTAMP                                           -- [DELTA RENAME] STATUS, LOAD_TIMESTAMP: renamed from RECORD_STATUS, ETL_LOAD_TIMESTAMP
    ) VALUES (
        src.PART_NUMBER, src.MANUFACTURER, src.WEIGHT_KG, src.CERTIFICATION_STATUS,
        src.LEAD_TIME_DAYS, src.UPDATED_AT, src.RISK_SCORE,
        src.SUPPLIER_RATING, src.PIPELINE_RUN_WATERMARK,
    ...
    UPDATE AEROSPACE_PARTS_TARGET
       SET STATUS         = 'Decommissioned',                           -- [DELTA RENAME] STATUS: was RECORD_STATUS, renamed to STATUS per BRD/STTM
           LOAD_TIMESTAMP = CURRENT_TIMESTAMP()                         -- [DELTA RENAME] LOAD_TIMESTAMP: was ETL_LOAD_TIMESTAMP, renamed to LOAD_TIMESTAMP per BRD/STTM
